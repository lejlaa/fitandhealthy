...
namespace FitAndHealthy
{
    public class FandHContext : DbContext
    {
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            //modelBuilder.Entity<Student>().HasRequired<Standard>(s => s.Standard).WithMany(s => s.Students).HasForeignKey(s => s.StdId);

            //modelBuilder.Entity<Person>().HasRequired(x => x.Team).WithMany(p => p.Persons).Map(m => m.MapKey("Team"));
            modelBuilder.Entity<Program>().HasRequired(x => x.Diet).WithMany(p => p.Programs).Map(m => m.MapKey("Diet"));
            modelBuilder.Entity<Program>().HasRequired(x => x.Author).WithMany(p => p.Programs).Map(m => m.MapKey("Author"));
